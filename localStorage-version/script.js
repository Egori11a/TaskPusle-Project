...
function addTask(taskName, taskDate = null) {
    if (!taskName.trim()) {
        showErrorMessage('Ты тупой еблан, пустой задачи не бывает!');
        return;
    }

    if(taskDate && new Date(taskDate) < new Date()) {
        showErrorMessage('Ты тупой еблан, дедлайн не может быть в прошлом!');
        return;
    }
...
function checkAllItems() {
    ...
    checkboxes.forEach((checkbox, index) => {
        ...
        todoElement.classList.toggle('completed', allTodos[index].completed);
    })
};
...
function updateTimer(index) {
    ...
    const todoItem = document.querySelector(`.todo:nth-child(${index + 1})`);
    if (!todoItem) return;
...
function editTask(index, taskElement, saveButton) {
    const todo = allTodos[index];
